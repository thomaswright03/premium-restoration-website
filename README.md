...
2. **Every privacy request** (email subject "Privacy request", or by phone): respond within `[PRIVACY RESPONSE PERIOD]`. Logging each request in a private spreadsheet (not in this public repo: date received, who asked, how identity was confirmed, what was found, action taken, date closed) is recommended. The Privacy Notice no longer promises a log; only add that promise back once the log exists.
