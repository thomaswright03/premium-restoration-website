...
| `materialsEstimator.enabled` | `true` shows "Pick Your Materials →" on a finished estimate card, using real scraped Home Depot prices (see "Materials picker" for how they're kept up to date).                                                                                                                                        |
...
**Uses real Home Depot prices**, generated by an offline scrape-and-commit pipeline rather than a live retailer API (there is no live API access — see "In progress" below). Controlled separately from the labor estimate by `materialsEstimator.enabled` (default `false`).

- **How the data gets in.** Two manual steps, neither of which runs automatically or from the live site:
  1. `python3 tools/scrapers/build_catalog.py` scrapes Home Depot search results for all 16 material categories and curates/normalizes them into `tools/scrapers/materials-catalog.json` (committed). See that script's own docstring for the real correctness issues it corrects for — tile/flooring are priced per box on Home Depot, not per sq ft; paint is priced per whatever pail size a listing happens to be, not per gallon — and for the outlier/mismatch filtering it applies (a broad search term occasionally returns a multi-fixture bundle or an adjacent-but-wrong product).
...
- **The finished materials card** shows itemised picks, a Materials Subtotal, a combined Labor + Materials line, and a **"Where to buy these"** shopping list with a real link per pick — followed by **Export as PDF** (same jsPDF pipeline as the labor estimate) and **Contact Us About This →** (prefills the Get a Quote form with the materials summary, same pattern as the labor estimate's `from=estimate`, here `from=materials`).
- **Category icons, not product photos.** Each option shows a plain inline SVG glyph for its category (toilet, tile, paint roller, etc.), defined in `js/script.js` as `MATERIAL_ICON_SVG`. The scraped data does carry a real image URL per product (see `homedepot_scraper.py`'s `image_url` field), but `CATALOG`'s `{id, name, retailers}` shape doesn't currently keep it, and `js/script.js` doesn't render one — wiring up real per-product photos is a follow-up, not done yet.
