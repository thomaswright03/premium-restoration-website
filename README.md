...
├── js/materials-pricing.js   materials picker catalog + logic — real scraped data (see "Materials picker")
...
| `materialsEstimator.enabled` | `true` folds a ZIP code + a real-product pick for every priced item straight into the estimate flow, ending in one combined labor + materials card, using real scraped Home Depot prices (see "Materials picker" for how they're kept up to date).                                                      |
...
**Uses real Home Depot prices**, generated by an offline scrape-and-commit pipeline rather than a live retailer API (there is no live API access — see "In progress" below). Turned on with `materialsEstimator.enabled` (default `false`).

- **How the data gets in.** Two manual steps, neither of which runs automatically or from the live site:
  1. `python3 tools/scrapers/build_catalog.py` scrapes Home Depot search results for all 16 material categories and curates/normalizes them into `tools/scrapers/materials-catalog.json` (committed), keeping each option's real product photo URL alongside its name and price. See that script's own docstring for the real correctness issues it corrects for — tile/flooring are priced per box on Home Depot, not per sq ft; paint is priced per whatever pail size a listing happens to be, not per gallon — and for the outlier/mismatch filtering it applies (a broad search term occasionally returns a multi-fixture bundle or an adjacent-but-wrong product).
  2. `node scripts/generate-materials-catalog.mjs` reads that JSON and rewrites the `CATALOG` block in `js/materials-pricing.js` in place, between `catalog:generated-start`/`-end` markers — the same generate-and-commit pattern `npm run pages` already uses for the HTML partials and published prices. `--check` (used in CI) exits non-zero if the file is stale.
  - Re-run both steps whenever prices should be refreshed; nothing does this on a schedule. `tools/scrapers/homedepot_scraper.py`'s own docstring explains why this stays a manual/offline tool rather than something run live from Vercel — it uses `cloudscraper` to get past Home Depot's Cloudflare protection, which is real Terms-of-Use risk, not something to run from shared/production infrastructure.
- **One continuous flow, not a separate step.** Once fixture counts are submitted, the same guided flow moves straight on — no "Pick Your Materials" button, no second card — into a ZIP code (used only for a placeholder regional price adjustment, `mockRegionalFactor()` — real market data, unlike the catalog, hasn't been connected here yet), then one grouped choice per material the labor estimate actually priced — read straight from `computeEstimate`'s own line items (`categoriesFromLines()`), so it can never offer a material for work that wasn't chosen, and never needs updating when the scope model changes. Picking a product also live-retints the matching fixture in the 3D preview (see "3D bathroom room preview" below). The flow ends in **one combined card**: labor lines, a Labor Subtotal, the picked material lines (each with its real photo), a Materials Subtotal, and one grand total — `materialsEstimator.enabled` off, or a scope/fixture combination with nothing to price a material for, falls back to the original labor-only card with its original copy unchanged.
- **Cheaper of two retailers, only when it's a real match.** An option with more than one entry in `retailers` would represent the _same manufacturer model_ sold at both Home Depot and Lowe's — `bestRetailer()` picks the lower price and shows "Cheaper than [other] ($X) for the same product." Every option currently has exactly one retailer (Home Depot), since only Home Depot is scraped today; the comparison logic is exercised by `tests/unit/materials.test.js`, ready for a second retailer if Lowe's is added later.
...
- **The combined card** shows itemised labor and material lines, a Labor Subtotal, a Materials Subtotal, one grand total, and a **"Where to buy the materials"** shopping list with a real link per pick — followed by **Export as PDF** (same jsPDF pipeline, one combined PDF) and **Contact Us About This →** (prefills the Get a Quote form with the combined summary, `from=estimate`).
- **Real product photos, not just category icons.** Each choice button and combined-card line shows the picked product's actual Home Depot photo (`CATALOG` options carry an `imageUrl`, threaded through from the scrape). A plain inline SVG glyph per category (toilet, tile, paint roller, etc., `js/script.js`'s `MATERIAL_ICON_SVG`) is only the fallback shown before a pick is made, or for any option whose scraped listing happened to have no image.

**In progress:** a Lowe's Developer Hub app (Product Discovery solution, Product Catalog capability) has been submitted for approval, as a path to a real (non-scraped) second retailer. `api/materials-options.js` is a not-yet-implemented Vercel serverless function stub for this — it validates its inputs and env vars and returns a clear "not configured" error, but makes no real API call yet, since Lowe's exact endpoint path, auth header, and response field names aren't visible until the app is approved and its interactive docs unlock. `.env.example` lists the environment variables it expects (`LOWES_API_KEY`, `LOWES_API_BASE_URL`) — copy it to `.env.local` for local testing, or set them as real Environment Variables in the Vercel project dashboard for production; both are gitignored so a real key never gets committed.
...
- **Two files, cleanly split.** `js/bathroom-room-layout.js` is DOM-free, Three.js-free pure logic (room-dimension defaulting, finish colors, and a deterministic fixture-placement algorithm), unit-tested in `tests/unit/room-layout.test.js`. `js/bathroom-room-3d.js` is the one Three.js scene module — the only first-party file using ES `import`/`export` (see the dedicated `eslint.config.js` block for it), self-hosted from `js/vendor/three/` (Three.js 0.186.1, MIT) via a native `<script type="importmap">` in `index.html` — no bundler, no CDN request at runtime. The vendored files are minified (Three.js's own npm package stops shipping pre-minified ESM builds — its unminified source alone is ~2.1MB, versus ~790KB minified, on a page every visitor loads): `npx esbuild <file> --minify --format=esm --outfile=<file>` was run once by hand against each vendored file (`three.core.js`, `three.module.js`, `addons/controls/OrbitControls.js`, `addons/environments/RoomEnvironment.js`), same "generate once, commit the result" pattern as the materials catalog — not an ongoing build step. Re-run it the same way after fetching a newer version.
- **Deterministic layout, not a real floor plan.** Fixtures (toilet, sink, bathtub, shower, vanity, cabinet, mirrors, entry door, shower shelf) are placed around the room's walls by a fixed-priority, fixed-scan-order algorithm — same inputs always produce the same layout, and changing one fixture's count never relocates an already-placed fixture of a different type. Every placement is checked against representative code-minimum clearances (`CLEARANCE_IN` in `js/bathroom-room-layout.js`) and the room's actual boundaries, not just wall-span packing; `computeLayout()` itself still just drops whatever doesn't fit (`droppedCounts`) rather than erroring or poking through a wall — the public chat flow now checks that ahead of time instead of letting a customer submit into it silently (see the next bullet and "blocked before it's silently dropped" below); the admin quoting tool has no 3D preview and so no such check, same as before.
- **Wall-click plumbing walls and entry points.** After fixture counts are entered, the customer can click directly on the 3D preview's walls to say which wall(s) carry the plumbing stack (restricting the toilet/sink/tub/shower to those walls, `plumbingWallIds` in `computeLayout()`) and to place one or more entry points (wall + click-to-nudge position + whether it has a door). Both steps are optional — skipping either leaves the layout unrestricted, same as before this existed. Raycasting/picking lives in `js/bathroom-room-3d.js` (`beginWallPicking`/`endWallPicking`); the chat sub-flow is in `js/script.js`'s `appendPlumbingWallsStep`/`appendEntryPointsStep`. Once this flow runs, it's the only source of the "Entry doors" count — the fixtures group leaves that field out entirely (an entry point counts unless its own "No — open archway" answer said otherwise); a doorless entry point renders as a trimmed open archway instead of a door slab. When the 3D preview is off there's no wall-click flow to ask this, so "Entry doors" stays a normal typed fixture count.
- **Blocked before it's silently dropped, not after.** Submitting fixture counts, or confirming an entry point's wall/position, runs the same clearance/overlap/anchor checks `computeLayout()` always has (`checkFit()` in `js/bathroom-room-3d.js`) against what's about to be committed — if anything wouldn't actually be placed, the submission is blocked with an inline error (which fixture, or "that spot doesn't fit") instead of quietly going through and only the 3D view showing fewer items than were priced.
- **Walk-in POV.** Once at least one entry point is placed, a "Walk in" button (next to the toilet style switcher) puts the camera at that entry point looking into the room, reusing the existing orbit controls for look-around (`setCameraMode`/`applyCameraMode`); a button row lets the customer switch between entry points when there's more than one.
- **Best-effort finish retinting, not real product geometry.** Picking a real product in the materials picker retints that fixture's proxy toward the product's actual finish colour — parsed from its name (`guessFinishColor()` in `js/materials-pricing.js`; matches words like "white," "matte black," "brushed nickel," "oil-rubbed bronze," falling back to `null`/no change when nothing matches) and applied per-instance via `setFixtureFinish()`/`applyFixtureFinish()` in `js/bathroom-room-3d.js`, which clones just the affected shared material so other fixtures of the same type aren't tinted too. This is deliberately an honest approximation: no major retailer exposes real per-SKU 3D models to scrape, so true "this exact product in 3D" isn't reachable from this data source — the room's real job is accurate scale/layout/clearance, not photorealism.
- **A real per-product scan path exists, but isn't wired in yet.** The only way to get a genuinely exact model of a real product (not an approximation) is a real 3D capture of the physical item — a phone/tablet LiDAR scan (`.usdz` from Polycam/Scaniverse/RealityScan/iOS Object Capture), a photogrammetry rig, or manufacturer CAD. `tools/scan-to-web/usdz_to_glb.py` converts a `.usdz` scan into a `.glb` Three.js can load (see its own docstring for the two-hop USD→OBJ→GLB path and why). It is **not** called from `js/bathroom-room-3d.js` — there's no loader vendored for it yet (would be `GLTFLoader` from `three/addons/loaders/`, not currently under `js/vendor/three/addons/`), and no per-product model URL field in the materials catalog. The blocker so far hasn't been the conversion — it's capture quality: a real test scan converted and rendered cleanly through the whole pipeline, but came out as ~6,000 disconnected mesh fragments covering a room-sized area rather than one clean fixture-sized surface, because it was a general scene-mode capture rather than the capture app's dedicated single-object "Object" mode (isolate the item, move slowly, go all the way around it). Mirrors and chrome/glossy fixtures are a separate, unavoidable hard limit for any depth-sensor or photogrammetry capture — reflective surfaces don't return real depth data no matter how carefully they're scanned. Once a clean single-object scan exists, wiring it in is: vendor `GLTFLoader` (+ its `three/addons/utils/BufferGeometryUtils.js` and `SkeletonUtils.js` dependencies), add a `modelUrl` to the relevant catalog entry, and load it in place of (or alongside) the procedural proxy in `rebuildFixtures()`.
...
- **Materials picker**: prices are real, but scraped from Home Depot's website rather than pulled from an official API or affiliate feed (see "Materials picker") — the owner should understand that comes with Terms-of-Use risk and that prices only refresh when someone manually re-runs the pipeline, before deciding `materialsEstimator.enabled` should stay on for real customers.
